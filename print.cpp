...
void print_results_GBM() {
    ...
    std::cout << std::setw(40) << "Value" << std::setw(30) << "Relative error" << std::setw(30) << "Half-Width"
              << std::setw(30) << "Computation Time" << std::endl;
    for (int i(0); i < 3; ++i) {
        std::vector<double> is_price = GBM::MC::ImportantSampling::call_price(S_0, K, r, sigma, T, M[i]);
        std::vector<double> is_delta = GBM::MC::ImportantSampling::call_delta(S_0, K, r, sigma, T, M[i], delta_S);
        std::vector<double> is_gamma = GBM::MC::ImportantSampling::call_gamma(S_0, K, r, sigma, T, M[i], delta_S);
        std::vector<double> is_vega = GBM::MC::ImportantSampling::call_vega(S_0, K, r, sigma, T, M[i], delta_sigma);
        std::cout << "\n-- Simulations: " << M[i] <<
...
void print_results_Heston(){
    ...
    for(int i(0); i< 3; ++i){
        ...
        std::vector<double> mc_vega_V_t  = Heston::MC::Direct::call_vega_V_t(S_0, K, r, kappa, theta, sigma, rho, T, M[i], delta_sigma);
        std::vector<double> mc_vega_V_0  = Heston::MC::Direct::call_vega_V_0(S_0, K, r, kappa, theta, sigma, rho, T, M[i], delta_sigma);
        std::vector<double> mc_vega_theta  = Heston::MC::Direct::call_vega_theta(S_0, K, r, kappa, theta, sigma, rho, T, M[i], delta_theta);
        std::cout << "\n-- Simulations: " << M[i] <<
                  ...
                  "\n   vega t:  " << std::setw(28) << mc_vega_V_t[0]  << std::setw(30) << "N/A"                                            << std::setw(30) << mc_vega_V_t[1]  << std::setw(30) << mc_vega_V_t[2]  <<
                  "\n   vega 0:  " << std::setw(28) << mc_vega_V_0[0]  << std::setw(30) << (mc_vega_V_0[0] - FFT_vega_V_0) / FFT_vega_V_0   << std::setw(30) << mc_vega_V_0[1]  << std::setw(30) << mc_vega_V_0[2]  <<
                  "\n   vega theta:  " << std::setw(24) << mc_vega_theta[0]  << std::setw(30) << "N/A"                                      << std::setw(30) << mc_vega_theta[1]  << std::setw(30) << mc_vega_theta[2]  <<
                  std::endl;
    }
    ...
    for(int i(0); i< 3; ++i){
        ...
        std::vector<double> mc_vega  = Heston::MC::Antithetic::call_vega_V_t(S_0, K, r, kappa, theta, sigma, rho, T, M[i], delta_sigma);
        std::cout << "\n-- Simulations: " << M[i] <<
                  "\n   price: " << std::setw(30) << mc_price[0] << std::setw(30) << (mc_price[0] - FFT_price) / FFT_price << std::setw(30) << mc_price[1] << std::setw(30) << mc_price[2] <<
                  "\n   delta: " << std::setw(30) << mc_delta[0] << std::setw(30) << (mc_delta[0] - FFT_delta) / FFT_delta << std::setw(30) << mc_delta[1] << std::setw(30) << mc_delta[2] <<
                  "\n   gamma: " << std::setw(30) << mc_gamma[0] << std::setw(30) << "N/A"                             << std::setw(30) << mc_gamma[1] << std::setw(30) << mc_gamma[2] <<
                  "\n   vega t:  " << std::setw(28) << mc_vega[0]  << std::setw(30) << "N/A"                             << std::setw(30) << mc_vega[1]  << std::setw(30) << mc_vega[2]  <<
                  std::endl;
    }
...
void print_graphs_both(){
    ...
    for(int i(0); i < 11; ++i){
        double it_rho = rho_range[i];
        Plot *plt_2 = new Plot_Heston(Spot, S_range[0], S_range[1], N);
        plt_2->set_values(MC, Antithetic, S_0, K, r, kappa, theta, sigma, it_rho, T, M[1], delta_S, delta_sigma);

        show_both_price(*plt_1, *plt_2, rho_range[i]);
        show_both_delta(*plt_1, *plt_2, rho_range[i]);
        show_both_gamma(*plt_1, *plt_2, rho_range[i]);
        show_both_vega (*plt_1, *plt_2, rho_range[i]);
    }
}
